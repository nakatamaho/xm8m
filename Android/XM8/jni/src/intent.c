...
//
extern char intent_buffer[4096 * 3];

//
// nativeIntent()
// pass intent data to application through JNI
//
void Java_net_retropc_pi_XM8_nativeIntent(JNIEnv* env, jclass jcls, jstring file_name)
{
	const char *src;
	unsigned char *dest;
	unsigned char high;
	...
	__android_log_print(ANDROID_LOG_VERBOSE, "XM8" ,"nativeIntent() path=""\x22""%s""\x22", path);

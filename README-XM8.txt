...
また実行時に必要となるSDL 2.0のDLLは、ライセンスに従って再配布しています。
ライセンスは"README-SDL.txt"を参照してください。

※version 1.10から、SDL 2.0.3をベースに独自パッチを当てた版に変更しました。
これはWindows 8.xでタッチイベントを正しく判定できない問題に対処するための
ものです。
...
基本的にソースファイルからのビルドが必要ですが、作者環境であるXUbuntu 14.04
...
ただしGoogle Playではなくapkファイルでの配布となりますので、以下の要領で
...
てください。具体的なパスは機種により異なりますが、以下に例を挙げます。
...
スクリーン右上の部分（ソフトウェアキーボードのMENU）を左クリックするとユー
ザI/Fに入ります。右クリックで一つ上のレベルまたはユーザI/Fから抜けます。
...
タッチ操作の他、[BACK]で一つ上のレベルまたはユーザI/Fから抜けます。
...
デフォルトの半透明レベルは128(50%)に設定しています。利用するアプリケーショ
...
"ES File Explorer""ASTRO File Manager"といったファイラーアプリでディスクイ
メージファイルを指定すると、上記Windows/Linuxと同様の動作を行います。
...
日本語レイアウトキーボードで、"変換""無変換""カタカナ"などの日本語変換特有
のキーは、全て機能しません。これはSDL 2.0.3の実装による制限です。


(2)サンプリングレート

Androidでは、48000Hzを超えるサンプリングレート(55467Hz, 88200Hz, 96000Hz)を
使用できないため、"Audio Options"で表示されません。これはSDL 2.0.3の実装に
よる制限です。
...
原則として"Keyboard enable"と"Joystick enable"は排他で使用してください。
また、デバイスによっては初回入力が無視され（このときに内部で登録）それ以降
キーが効くものもあります。この動作は仕様です。


デバイスによっては音切れが度々発生する場合があります。まずは"Audio Options"
